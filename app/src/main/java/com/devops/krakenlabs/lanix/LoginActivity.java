...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor>,
    SessionNotifier, ControllerNotifier, ActivityCompat.OnRequestPermissionsResultCallback {
  ...
  String[] PERMISSIONS = {android.Manifest.permission.WRITE_CONTACTS,
      android.Manifest.permission.READ_CONTACTS, android.Manifest.permission.ACCESS_FINE_LOCATION,
      android.Manifest.permission.READ_PHONE_STATE, android.Manifest.permission.INTERNET,
  ...
  private void openPrivacidad() {
    ...
    recoverPwFragment.show(getSupportFragmentManager(), recoverPwFragment.getClass()
        .getSimpleName());
  }
  ...
  private void addEmailsToAutoComplete(List<String> emailAddressCollection) {
    Log.d(TAG, "addEmailsToAutoComplete() called with: emailAddressCollection = ["
  ...
  public void sessionComplete() {
    ...
    try {
      if (authController.getUser() != null && (authController.getUser().getError().getNo() == 0)) {
        ...
        Log.e(TAG, "sessionComplete: " + authController.getError());
        if (authController.getErrors() != null) {
          mEmailView.setError(authController.getErrors().getDescripcion());
          mEmailView.requestFocus();
        } else if (null == authController.getUser()) {
          new AlertDialog.Builder(LoginActivity.this)
              .setMessage("No hay conexión a internet disponible.").create().show();
  ...
  private void saveCredentials(String user, String pw, Boolean isPermanent) {
    ...
    SharedPreferences loginPreferences = getSharedPreferences(SPF_NAME, Context.MODE_PRIVATE);
    loginPreferences.edit().putString(USERNAME, user).putString(PASSWORD, pw)
        .putBoolean(SAVED, isPermanent).commit();
  }
  ...
  public void tokenDeviceComplete() {
    ...
    Log.e(TAG, "tokenDeviceComplete: " + authController.getDevice().getVersionApp() + " === "
        + requestVersion());
    // if (authController.getDevice().getError().getNo() == 0 &&
    // !authController.getDevice().getVersionApp().equals(requestVersion())){//Version más nueva de
    // la aplicacion
  ...
  private void openBrowser() {
    try {
      Intent browserIntent =
          new Intent(Intent.ACTION_VIEW, Uri.parse(authController.getDevice()
              .getRutaDeDescargaApp()));
